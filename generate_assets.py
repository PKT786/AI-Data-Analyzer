...
# ------------------------------------------------------------- auth hero --
def make_auth_hero(path="assets/images/auth_hero.png", w=900, h=1200):
    """Vertical/portrait hero used on the split-screen login page (brand
    panel on the left, login/signup form on the right)."""

    base = _vertical_gradient((w, h), NAVY_DARK, INDIGO).convert("RGBA")

    base.alpha_composite(_soft_glow((w, h), (w * 0.80, h * 0.12), int(w * 0.75), VIOLET, 130))
    base.alpha_composite(_soft_glow((w, h), (w * 0.15, h * 0.92), int(w * 0.70), TEAL, 65))
    base.alpha_composite(_soft_glow((w, h), (w * 0.50, h * 0.50), int(w * 0.55), GOLD, 35))
    ...
    ring_layer = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    rd = ImageDraw.Draw(ring_layer)
    for i, radius in enumerate([420, 320, 230]):
        alpha = 45 - i * 10
        cx, cy = w * 0.90, h * 0.06
        rd.ellipse([cx - radius, cy - radius, cx + radius, cy + radius],
                   outline=GOLD + (alpha,), width=3)
    ...
    # ---- brand mark (logo badge + wordmark) near the top ----
    badge_size = int(w * 0.16)
    badge = _diagonal_gradient((badge_size, badge_size), INDIGO, VIOLET).convert("RGBA")
    bm = Image.new("L", (badge_size, badge_size), 0)
    bmd = ImageDraw.Draw(bm)
    bmd.ellipse([0, 0, badge_size, badge_size], fill=255)
    bx, by = int(w * 0.07), int(h * 0.055)
    base.paste(badge, (bx, by), bm)
    bd = ImageDraw.Draw(base)
    ring_w = max(2, int(badge_size * 0.02))
    bd.ellipse([bx + ring_w, by + ring_w, bx + badge_size - ring_w, by + badge_size - ring_w],
               outline=GOLD + (230,), width=ring_w)
    mono_font = _font(POPPINS_BOLD, int(badge_size * 0.40))
    mb = bd.textbbox((0, 0), "PTH", font=mono_font)
    mw, mh = mb[2] - mb[0], mb[3] - mb[1]
    bd.text((bx + (badge_size - mw) / 2 - mb[0], by + (badge_size - mh) / 2 - mb[1]),
             "PTH", font=mono_font, fill=WHITE + (255,))

    word_font = _font(POPPINS_SEMI, int(badge_size * 0.30))
    wx = bx + badge_size + int(w * 0.03)
    wy = by + badge_size * 0.20
    d.text((wx, wy), "AI DATA ANALYZER", font=word_font, fill=WHITE + (255,))
    pro_font = _font(POPPINS_BOLD, int(badge_size * 0.30))
    wb = d.textbbox((0, 0), "AI DATA ANALYZER", font=word_font)
    d.text((wx, wy + (wb[3] - wb[1]) + int(badge_size * 0.06)), "PRO",
             font=pro_font, fill=GOLD_BRIGHT + (255,))

    # ---- headline ----
    title_font = _font(POPPINS_BOLD, int(w * 0.105))
    sub_font = _font(POPPINS_REG, int(w * 0.042))
    ty = int(h * 0.20)
    tx = int(w * 0.07)
    d.text((tx, ty), "Turn Raw Data", font=title_font, fill=WHITE + (255,))
    d.text((tx, ty + int(w * 0.115)), "Into Decisions", font=title_font, fill=GOLD_BRIGHT + (255,))
    d.text((tx, ty + int(w * 0.115) * 2 + int(w * 0.01)),
             "AI cleaning \u2022 dashboards \u2022 insights \u2022 reports",
             font=sub_font, fill=MIST + (255,))

    # ---- floating dashboard mockup card (lower half) ----
    card_w, card_h = int(w * 0.86), int(h * 0.40)
    pad = 30
    card_layer = Image.new("RGBA", (card_w + pad * 2, card_h + pad * 2), (0, 0, 0, 0))

    shadow = Image.new("RGBA", card_layer.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shadow)
    sd.rounded_rectangle([pad + 10, pad + 20, pad + card_w + 10, pad + card_h + 20],
                          radius=24, fill=(0, 0, 0, 150))
    shadow = shadow.filter(ImageFilter.GaussianBlur(20))
    card_layer.alpha_composite(shadow)

    cd = ImageDraw.Draw(card_layer)
    cd.rounded_rectangle([pad, pad, pad + card_w, pad + card_h], radius=22, fill=(250, 251, 253, 255))

    header_h = int(card_h * 0.16)
    header_grad = _diagonal_gradient((card_w, header_h), NAVY, INDIGO).convert("RGBA")
    header_mask = Image.new("L", (card_w, header_h), 0)
    hmd = ImageDraw.Draw(header_mask)
    hmd.rounded_rectangle([0, 0, card_w, header_h + 24], radius=22, fill=255)
    hmd.rectangle([0, header_h - 24, card_w, header_h], fill=255)
    card_layer.paste(header_grad, (pad, pad), header_mask)

    dot_colors = [(255, 95, 86), (255, 189, 46), (39, 201, 63)]
    for i, c in enumerate(dot_colors):
        r = header_h * 0.11
        cx_ = pad + int(card_w * 0.05) + i * int(r * 3)
        cy_ = pad + header_h // 2
        cd.ellipse([cx_ - r, cy_ - r, cx_ + r, cy_ + r], fill=c + (255,))

    body_x = pad + int(card_w * 0.05)
    body_w = card_w - int(card_w * 0.10)
    body_y = pad + header_h + int(card_h * 0.06)

    kpi_h = int(card_h * 0.20)
    kpi_gap = int(card_w * 0.03)
    kpi_w = (body_w - kpi_gap * 2) / 3
    kpi_data = [("Revenue", "$482K", GREEN), ("Growth", "+18%", BLUE), ("Churn", "2.1%", GOLD)]
    kpi_label_font = _font(POPPINS_REG, int(kpi_h * 0.22))
    kpi_value_font = _font(POPPINS_BOLD, int(kpi_h * 0.34))
    for i, (label, value, color) in enumerate(kpi_data):
        kx = body_x + i * (kpi_w + kpi_gap)
        cd.rounded_rectangle([kx, body_y, kx + kpi_w, body_y + kpi_h], radius=10,
                              fill=(244, 246, 250, 255))
        cd.rectangle([kx, body_y, kx + int(kpi_w * 0.06), body_y + kpi_h], fill=color + (255,))
        cd.text((kx + kpi_w * 0.14, body_y + kpi_h * 0.14), label, font=kpi_label_font,
                 fill=(110, 118, 138, 255))
        cd.text((kx + kpi_w * 0.14, body_y + kpi_h * 0.48), value, font=kpi_value_font,
                 fill=(30, 38, 60, 255))

    chart_y = body_y + kpi_h + int(card_h * 0.06)
    chart_h = card_h - (chart_y - pad) - int(card_h * 0.06)
    cd.rounded_rectangle([body_x, chart_y, body_x + body_w, chart_y + chart_h], radius=10,
                          fill=(244, 246, 250, 255))
    bars = [0.35, 0.55, 0.42, 0.72, 0.60, 0.88, 0.68, 0.5, 0.8]
    n = len(bars)
    inner_pad = body_w * 0.04
    slot = (body_w - inner_pad * 2) / n
    bar_colors = [BLUE, GREEN, GOLD, VIOLET]
    base_y = chart_y + chart_h - inner_pad * 0.6
    max_bar_h = chart_h - inner_pad
    for i, frac in enumerate(bars):
        bx0 = body_x + inner_pad + i * slot + slot * 0.16
        bx1 = body_x + inner_pad + i * slot + slot * 0.84
        by1 = base_y
        by0 = base_y - max_bar_h * frac
        cd.rounded_rectangle([bx0, by0, bx1, by1], radius=5, fill=bar_colors[i % len(bar_colors)] + (255,))

    rotated = card_layer.rotate(-2.4, resample=Image.BICUBIC, expand=True)
    rx = int(w * 0.06)
    ry = int(h * 0.52)
    base.alpha_composite(rotated, (rx, ry))

    # trust badges at the very bottom
    chips = ["\u2713 Secure Login", "\u2713 Free to Start"]
    tag_font = _font(POPPINS_SEMI, int(w * 0.032))
    cx = int(w * 0.07)
    cy = int(h * 0.945)
    for label in chips:
        bb = d.textbbox((0, 0), label, font=tag_font)
        lw = bb[2] - bb[0] + int(w * 0.045)
        lh = int(h * 0.045)
        d.rounded_rectangle([cx, cy, cx + lw, cy + lh], radius=lh // 2, outline=GOLD + (255,), width=2)
        d.text((cx + (lw - (bb[2] - bb[0])) / 2 - bb[0], cy + (lh - (bb[3] - bb[1])) / 2 - bb[1]),
                 label, font=tag_font, fill=GOLD_BRIGHT + (255,))
        cx += lw + int(w * 0.025)

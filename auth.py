...
auth.py
-------
Authentication layer for the Premium ATS Resume Builder.
...
will simply no-op (log a warning) instead of crashing the app.
...
gracefully if its keys are missing):
    ...
    account_sid   = "ACxxxx"
    auth_token    = "xxxx"
    from_number   = "+1xxxxxxxxxx"          # a Twilio phone number you own
...
def _send_admin_email(name: str, email: str, mobile: str, event: str) -> None:
    ...
    msg = MIMEText(body)
    msg["Subject"] = f"[PTH Resume Builder] {event}: {name}"
    msg["From"] = sender
    msg["To"] = admin_email
...
def _send_admin_sms(name: str, email: str, mobile: str, event: str) -> None:
    ...
    body = f"[PTH Resume Builder] {event}: {name} ({email}, {mobile})"
